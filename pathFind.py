...
def hop_fun(path_size):
    if np.random.rand() <= 0.1 and path_size < 25:
        return 0.2 * np.sqrt(path_size)
    return 1
...
def Dijkstra_old(G, source, target, amt, cost_function):
    paths = {}
    dist = {}
    delay = {}
    amount =  {}
...
def Eclair(G, source, target, amt, path=None):
    ...
    B = nd.nested_dict()
    if (path == None):
        B[0],d,c,di = Dijkstra_old(G, source, target, amt, eclair_cost_fun)
        #print(B[0],di)
    # print(B[0]["Path"])
    else:
        B[0] = path
    ...
    for k in range(1, 3):
        ...
        for i in range(0, len(A) - 2):
            ...
            spurpath, delay, cost, dist = Dijkstra_old(G1, spurnode, target, amt, eclair_cost_fun)
                ...
                if (rootpathnode != spurnode):
                    if (G1.has_node(rootpathnode)):
                        for [u, v] in G1.copy().in_edges(rootpathnode):
                            ...
                            edges_removed.append((v, u))
            # print(eclair_single(G1,spurnode,target,amt))
            spurpath, delay, cost, dist = Dijkstra_old(G1, source, spurnode, amt_spur, eclair_cost_fun)
...
def Dijkstra_general_old(G, source, target, amt, cost_function):
    paths = {}
    paths1 = {}
    paths2 = {}
    dist = {}
