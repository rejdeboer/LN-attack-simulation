...
def hop_fun(path_size):
    if np.random.rand() <= HOP_FACTOR:
        return 0.1 * np.sqrt(path_size)
    return 1
...
def Dijkstra(G,source,target,amt,cost_function):
    ...
    while 0!=pq.qsize():
        ...
        #for [v, curr] in G.in_edges(curr):
            #if G.out_degree(v) > max_degree and v not in visited:
                #max_degree = G.out_degree(v)
        for [v,curr] in G.in_edges(curr):
            if v == source and G.edges[v,curr]["Balance"]>=amount[curr]:
                cost = (dist[curr] + amount[curr]*G.edges[v,curr]["Delay"]*LND_RISK_FACTOR) * hop_fun(len(paths[curr]))
                if cost < dist[v]:
                    dist[v] = cost
            ...
            if(G.edges[v, curr]["Balance"] + G.edges[curr, v]["Balance"] >= amount[curr]) and v not in visited and v!=source:
                cost = (dist[curr] + cost_function(G,amount[curr],curr,v)) * hop_fun(len(paths[curr]))
                if cost < dist[v]:
                    dist[v] = cost
                    paths[v] = [v] + paths[curr]
                    delay[v] = G.edges[v,curr]["Delay"] + delay[curr]
        ...
        for i in range(0, len(t)):
            if(d[i] == 0):
                ...
                if (len(path) == len(set(path))):
                    amt = a[i]
                    pot = path[len(path) - 1]
                    sources_old = deanonymize_old(G, pot, path, amt, lnd_cost_fun)
                    sources = deanonymize_new(G, pot, path, amt, lnd_cost_fun)
                    if sources_old != None:
                        anon_sets_old[pot] = list(sources_old)
                    if sources != None:
                        anon_sets[pot] = list(sources)
        level = level - 1
    return anon_sets_old,anon_sets,flag1

def deanonymize_old(G, target, path, amt, cost_function):
    pq = PriorityQueue()
    delays = {}
    costs = {}
...
def deanonymize_new(G, target, path, amt, cost_function):
    pq = PriorityQueue()
    delays = {}
    costs = {}
    paths = nd.nested_dict()
        ...
        if(curr == pre):
            ...
            for [v, curr] in G.in_edges(curr):
                if v != adv and v != nxt:
                    sources.append(v)
        if flag1 == 1 and flag2 == 1:
            if pre in paths[curr] or nxt in paths[curr] or adv in paths[curr]:
                for [v,curr] in G.in_edges(curr):
                    sources.append(curr)
    sources = set(sources)
    return sources
...
def route(G,path,delay,amt,ads,amt1,file):
    ...
    if len(path) == 2:
        ...
        transaction = {"sender": path[0], "recipient": path[1], "path" : path, "delay": delay, "amount":amt1,
                    ...
                    "success":True,"anon_sets_old":anon_sets_old,"anon_sets":anon_sets,"comp_attack":comp_attack}
    ...
    while(i < len(path)-1):
        ...
        if path[i] in ads:
            ...
            B_old, B, flag = dest_reveal_new(G1,path[i],delay1,amt,path[i-1],path[i+1])
            for j in B_old:
                dest = {j:B_old[j]}
                dests_old.append(dest)
            for j in B:
                dest = {j:B[j]}
                dests.append(dest)
            if flag == True:
            ...
            anon_sets.append({path[i]:dests})
            anon_sets_old.append({path[i]:dests_old})
        if(G.edges[path[i],path[i+1]]["Balance"] >= amt):
            ...
            transaction = {"sender": path[0], "recipient": path[len(path)-1], "path": path, "delay": delay,
                           "amount": amt1, "cost": cost, "attacked": attacked,
                           "success": False, "anon_sets_old": anon_sets_old,"anon_sets":anon_sets, "comp_attack": comp_attack}
            transactions.append(transaction)
            return False

G = nx.barabasi_albert_graph(500,2,65)
#G = nx.fast_gnp_random_graph(500, 0.2)
G = nx.DiGraph(G)
...
for [u,v] in G.edges():
    ...
    G.edges[u,v]["Balance"] = rn.randint(500,20000)
