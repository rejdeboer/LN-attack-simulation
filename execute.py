...
import attack_mixed_old as at_old
...
def route(G,path,delay,amt,ads,amt1,file,tech):
    ...
    if len(path) == 2:
        ...
        transaction = {"sender": path[0], "recipient": path[1], "path" : path, "delay": delay, "amount":amt1, "cost": cost, "tech":tech,
                       "attacked":0,
                    "success":True, "anon_sets": anon_sets,"anon_sets_old":anon_sets_old,"comp_attack":comp_attack}
    ...
    while(i < len(path)-1):
        ...
        if path[i] in ads:
            ...
            print(delay1)
            B,flag = at_old.dest_reveal_new(G1, path[i], delay1, amt, path[i - 1], path[i + 1])
            for j in B:
                dest = {j:B[j]}
                dests.append(dest)
            if flag == True:
        ...
        if(G.edges[path[i],path[i+1]]["Balance"] >= amt):
            ...
            transaction = {"sender": path[0], "recipient": path[len(path)-1], "path": path, "delay": delay,
                               "amount": amt1, "cost": cost,"tech":tech, "attacked": attacked,
                               "success": False, "anon_sets": anon_sets, "anon_sets_old": anon_sets_old, "comp_attack": comp_attack}
            transactions.append(transaction)
