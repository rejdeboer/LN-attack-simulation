...
sing_any = 0
ads = [3, 2, 7, 25, 5, 6, 11, 52, 1, 34, 22, 10, 16, 39, 18, 12, 68, 37, 13, 19, 15, 9, 75, 32, 8, 28, 45, 29, 95, 20, 99, 41, 56, 84, 85, 108, 103, 36, 24, 98, 92, 35, 89, 33, 143, 31, 27, 104, 47, 51]
                            ...
                            for adv in ad:
                                ...
                                for dest in ad[adv]:
                                    for rec in dest:
                                        ...
                                        if k["sender"] not in dest[rec]:
                                            source_false_positives_old += 1
                                        if int(rec) == k["recipient"]:
                                            rec_found_old += 1
                                            if k["sender"] in dest[rec]:
                                                pair_found_old += 1
                                            else:
                                                pairwise_source_false_positives_old += 1
                                        num_sources_old += len(dest[rec])
...
print(f'Correct singular destination ratio (sanity check): {correct_sing_dest_ratio} ({perc(correct_sing_dest_ratio)}%)')
...
print(f'Destination false positive ratio: {destination_false_positive_ratio} ({perc(destination_false_positive_ratio)}%)')
print(f'Source false positive ratio: {source_false_positive_ratio} ({perc(source_false_positive_ratio)}%)')
print(f'Pairwise source false positive ratio: {pairwise_source_false_positive_ratio} ({perc(pairwise_source_false_positive_ratio)}%)')
